...
export const Restock_in = () => {
  ...
  const columns = [
    ...
    { key: 'importRequestCode', label: 'Import Request code', width: 'whitespace-nowrap min-w-[170px]' },
    { key: 'importCommandCode', label: 'Import Command code', width: 'whitespace-nowrap min-w-[170px]' },
    { key: 'dateCreate', label: 'Date Create', width: 'w-24' },
    { key: 'importWarehouse', label: 'Import warehouse', width: 'whitespace-nowrap min-w-[150px]' },
    { key: 'contract', label: 'Contract', width: 'whitespace-nowrap' },
    { key: 'creator', label: 'Creator', width: 'whitespace-nowrap' },
    { key: 'unitRequests', label: 'Unit Requests', width: 'whitespace-nowrap' },
    { key: 'unitReceive', label: 'Unit Receive', width: 'whitespace-nowrap' },
                ...
                <table className="min-w-full divide-y divide-rose-100 text-left text-xs bg-white">
                  <thead className="bg-rose-50/50 text-rose-900 font-bold uppercase tracking-wider">
                    <tr>
                      ...
                      <th className="px-4 py-3">Import Request code</th>
                      <th className="px-4 py-3">Unit Requests</th>
                  ...
                  <tbody className="divide-y divide-rose-100">
                    {filteredAlarmItems.map((item, idx) => (
                      <tr key={item.id} className="hover:bg-rose-50/30 transition-colors">
                        ...
                        <td className="px-4 py-3 font-mono font-semibold text-gray-800">{item.importRequestCode || '-'}</td>
                        <td className="px-4 py-3 text-gray-700">{item.unitRequests || '-'}</td>
                  ...
                  <tr key={item.id} className={`transition-colors ${isAlarm ? 'bg-rose-50/90 font-semibold' : selectedRows.has(item.id) ? 'bg-blue-50/90' : 'even:bg-slate-50/70 odd:bg-white hover:bg-blue-50/70'}`}>
                    ...
                    <td className="border border-slate-300 px-1.5 py-0.25 font-mono font-bold text-slate-900 whitespace-nowrap min-w-[170px]">
                      {editingCell?.id === item.id && editingCell?.field === 'importRequestCode' ? (
                        <input type="text" defaultValue={item.importRequestCode} autoFocus onBlur={(e) => saveEdit(item.id, 'importRequestCode', e.target.value)} onKeyDown={(e) => handleKeyPress(e, item.id, 'importRequestCode')} className="w-full px-1 py-0 border border-blue-500 rounded font-mono text-[9.5px] bg-white" />
                      ) : (
                        <div onClick={() => startEdit(item.id, 'importRequestCode', item.importRequestCode)} className="cursor-pointer hover:bg-slate-200/60 px-1 py-0 rounded font-mono">{item.importRequestCode || '-'}</div>
                      )}
                    </td>
                    <td className="border border-slate-300 px-1.5 py-0.25 font-mono font-bold text-slate-900 whitespace-nowrap min-w-[170px]">
                      {editingCell?.id === item.id && editingCell?.field === 'importCommandCode' ? (
                        <input type="text" defaultValue={item.importCommandCode} autoFocus onBlur={(e) => saveEdit(item.id, 'importCommandCode', e.target.value)} onKeyDown={(e) => handleKeyPress(e, item.id, 'importCommandCode')} className="w-full px-1 py-0 border border-blue-500 rounded font-mono text-[9.5px] bg-white" />
                      ) : (
                        <div onClick={() => startEdit(item.id, 'importCommandCode', item.importCommandCode)} className="cursor-pointer hover:bg-slate-200/60 px-1 py-0 rounded font-mono">{item.importCommandCode || '-'}</div>
                      )}
                    </td>
                    ...
                    <td className="border border-slate-300 px-1.5 py-0.25 text-slate-800 font-bold whitespace-nowrap min-w-[150px]">
                      {editingCell?.id === item.id && editingCell?.field === 'importWarehouse' ? (
                        <input type="text" defaultValue={item.importWarehouse} autoFocus onBlur={(e) => saveEdit(item.id, 'importWarehouse', e.target.value)} onKeyDown={(e) => handleKeyPress(e, item.id, 'importWarehouse')} className="w-full px-1 py-0 border border-blue-500 rounded bg-white" />
                      ) : (
                        <div onClick={() => startEdit(item.id, 'importWarehouse', item.importWarehouse)} className="cursor-pointer hover:bg-slate-200/60 px-1 py-0 rounded">{getWarehouseBadge(item.importWarehouse)}</div>
                      )}
                    </td>
                    <td className="border border-slate-300 px-1.5 py-0.25 text-slate-800 whitespace-nowrap">
                      {editingCell?.id === item.id && editingCell?.field === 'contract' ? (
                        <input type="text" defaultValue={item.contract} autoFocus onBlur={(e) => saveEdit(item.id, 'contract', e.target.value)} onKeyDown={(e) => handleKeyPress(e, item.id, 'contract')} className="w-full px-1 py-0 border border-blue-500 rounded bg-white" />
                      ) : (
                        <div onClick={() => startEdit(item.id, 'contract', item.contract)} className="cursor-pointer hover:bg-slate-200/60 px-1 py-0 rounded">{item.contract || '-'}</div>
                      )}
                    </td>
                    <td className="border border-slate-300 px-1.5 py-0.25 text-slate-800 whitespace-nowrap">
                    ...
                    <td className="border border-slate-300 px-1.5 py-0.25 text-slate-800 whitespace-nowrap">
                      {editingCell?.id === item.id && editingCell?.field === 'unitRequests' ? (
                        <input type="text" defaultValue={item.unitRequests} autoFocus onBlur={(e) => saveEdit(item.id, 'unitRequests', e.target.value)} onKeyDown={(e) => handleKeyPress(e, item.id, 'unitRequests')} className="w-full px-1 py-0 border border-blue-500 rounded bg-white" />
                      ) : (
                        <div onClick={() => startEdit(item.id, 'unitRequests', item.unitRequests)} className="cursor-pointer hover:bg-slate-200/60 px-1 py-0 rounded">{item.unitRequests || '-'}</div>
                      )}
                    </td>
                    <td className="border border-slate-300 px-1.5 py-0.25 text-slate-800 whitespace-nowrap">
                      {editingCell?.id === item.id && editingCell?.field === 'unitReceive' ? (
                        <input type="text" defaultValue={item.unitReceive} autoFocus onBlur={(e) => saveEdit(item.id, 'unitReceive', e.target.value)} onKeyDown={(e) => handleKeyPress(e, item.id, 'unitReceive')} className="w-full px-1 py-0 border border-blue-500 rounded bg-white" />
                      ) : (
                        <div onClick={() => startEdit(item.id, 'unitReceive', item.unitReceive)} className="cursor-pointer hover:bg-slate-200/60 px-1 py-0 rounded">{item.unitReceive || '-'}</div>
                      )}
                    </td>
                    <td className="border border-slate-300 px-1.5 py-0.25 font-mono text-center text-slate-700 whitespace-nowrap">
                      {editingCell?.id === item.id && editingCell?.field === 'dateDelivery' ? (
                        <input type="text" defaultValue={item.dateDelivery} autoFocus onBlur={(e) => saveEdit(item.id, 'dateDelivery', e.target.value)} onKeyDown={(e) => handleKeyPress(e, item.id, 'dateDelivery')} className="w-full px-1 py-0 border border-blue-500 rounded font-mono text-[9.5px] text-center bg-white" />
                      ) : (
                        <div onClick={() => startEdit(item.id, 'dateDelivery', item.dateDelivery)} className="cursor-pointer hover:bg-slate-200/60 px-1 py-0 rounded font-mono">{item.dateDelivery || '-'}</div>
                      )}
                    </td>
                ...
                <tr>
                  <td colSpan={columns.length + 1} className="border border-slate-300 px-6 py-12 text-center text-slate-400 font-bold text-sm bg-white">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-xs text-slate-500">Filters: YCNKGIS_ + Unsigned + Valid Units</p>
                    </div>
                  </td>
                </tr>
              )}
