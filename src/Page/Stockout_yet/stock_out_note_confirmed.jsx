        ...
        <div className="bg-white rounded-2xl shadow-2xl max-w-7xl w-full mx-4 max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <div className="p-6 overflow-y-auto flex-1">
            ...
            <div className="flex gap-2 mb-4 border-b pb-2">
              <button onClick={() => setKpiViewMode('all')} className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${kpiViewMode === 'all' ? 'bg-slate-900 text-white font-semibold shadow-xs' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>📋 All ({calculateKPIData.allData.length})</button>
              <button onClick={() => setKpiViewMode('active')} className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${kpiViewMode === 'active' ? 'bg-slate-700 text-white font-semibold shadow-xs' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>🔄 Active</button>
              <button onClick={() => setKpiViewMode('completed')} className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${kpiViewMode === 'completed' ? 'bg-indigo-600 text-white font-semibold shadow-xs' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>✅ Completed</button>
            </div>
        ...
        <div className="bg-white px-6 py-5 border-b border-slate-100">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <p className="text-rose-600 mt-1 text-xs font-bold uppercase tracking-wider">TEAM STEP 3</p>
              <p className="text-slate-500 mt-1 text-sm font-normal">តាមដានសម្ភារៈដែលបានបង្កើតការប្រគល់ (Create Hand Over) រួចហើយ ប៉ុន្តែក្រុមការងារមិនទាន់បានទទួល និងបញ្ជាក់ (Confirm) នៅឡើយ។</p>
            </div>
            <div className="flex gap-2">
