...
// Storage Keys for Metfone Net Module 03
const STORAGE_KEYS = {
  DATA: 'metfone_handover_data',
  COMPLETION: 'metfone_handover_completionHistory',
  TARGETS: 'metfone_handover_targets',
  TARGET_HISTORY: 'metfone_handover_targetHistory',
  CONFIRMED: 'metfone_handover_confirmedStatus'
};

// Initial Raw Metfone Hand Over Data (Empty by default)
export const INITIAL_M3_DATA = [];
...
const getUnitFromRecipient = (recipient, code) => {
  ...
  // Province match from BBBG_MFNET_GIS_CHA_FBC02 or MFNet_GIS_CHA_FBC02
...
const HandOverYetConfirmMetfone = () => {
  ...
  useEffect(() => {
    const fetchDbData = async () => {
      ...
      if (dbData && dbData.length > 0) {
        // Requirement: Unit confirm handover ចាប់យកតែ GIS
        ...
        const enriched = filtered.map(item => ({
          ...
          unit: getUnitFromRecipient(item.unitConfirm, item.code),
  ...
  const columns = [
    ...
    { key: 'code', label: 'Code of handover minutes', width: 'whitespace-nowrap min-w-[200px]', align: 'text-left' },
    { key: 'type', label: 'Type of handover', width: 'whitespace-nowrap min-w-[170px]', align: 'text-left' },
    { key: 'handoverUnit', label: 'Handover unit', width: 'whitespace-nowrap min-w-[150px]', align: 'text-left' },
    { key: 'unitConfirm', label: 'Unit confirm handover', width: 'whitespace-nowrap min-w-[160px]', align: 'text-left' },
    ...
    { key: 'status', label: 'Status', width: 'w-24', align: 'text-center' }
  ];
  ...
  const processImport = (newRawData) => {
    ...
    if (gisData.length === 0) {
      showNotification('⚠️ No valid "Not confirmed" GIS records found in the import data!', 'warning');
      return;
    }
    ...
    const processedNewData = gisData.map((item, index) => {
      const unit = getUnitFromRecipient(item.unitConfirm, item.code);
      ...
      if (cells.length >= 5) {
        ...
        if (cells.length - offset >= 6) {
          parsedRows.push({
            ...
            date: cells[offset + 4] || '',
            status: cells[offset + 5] || 'Not confirmed'
          });
        }
      }
    }
  ...
  const clearAllData = async () => {
    if (window.confirm('⚠️ Are you sure you want to clear ALL Metfone Hand Over Yet Confirm data?')) {
      setData([]);
  ...
  const exportToExcel = () => {
    const exportData = filteredData.map(item => ({
      'No': item.no,
      'Code of handover minutes': item.code,
      'Type of handover': item.type,
      'Handover unit': item.handoverUnit,
      'Unit confirm handover': item.unitConfirm,
      'Handover date': item.date,
      'Unit': item.unit,
      'Days': item.daysDiff,
      'TEAM': item.team || '-',
      'Status': item.status
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Hand Over Yet Confirm');
    XLSX.writeFile(wb, `METFONE_HAND_OVER_YET_CONFIRM_${new Date().toISOString().split('T')[0]}.xlsx`);
  };

  // Main Filtered Data (Unit confirm handover MUST contain GIS silently)
  const filteredData = useMemo(() => {
    let filtered = data.filter(item => {
      const confirmUnit = (item.unitConfirm || '').toUpperCase();
      return confirmUnit.includes('GIS');
    });
  ...
  const copyAlarmsToClipboard = () => {
    ...
    const text = filteredAlarmItems.map(item => 
      `${item.unit}\n| Code: ${item.code}\n📅 Date: ${item.date} | ⏰ Delay: +${item.daysDiff} days\nHandover: ${item.handoverUnit || '-'} → Confirm: ${item.unitConfirm || '-'}\nTEAM: ${item.team || '-'}\nStatus: ${item.status || '-'}`
    ).join('\n\n');
  ...
  const getUnitConfirmBadge = (unitConfirm) => {
    if (unitConfirm && unitConfirm.toUpperCase().includes('GIS')) {
      return <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold bg-emerald-100 text-emerald-800">{unitConfirm}</span>;
    }
    return <span className="text-slate-600">{unitConfirm}</span>;
  };
        ...
        <div className="bg-white rounded-2xl shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-hidden flex flex-col">
          <div className="bg-gradient-to-r from-indigo-900 via-blue-900 to-indigo-950 px-5 py-3 text-white flex justify-between items-center">
            <div className="flex items-center gap-2">
              <span className="text-2xl">📊</span>
              <div>
                <h2 className="text-base font-black">SYSTEM METFONE NET — HAND OVER YET CONFIRM KPI</h2>
        ...
        <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full flex flex-col overflow-hidden">
          <div className="bg-gradient-to-r from-emerald-800 to-teal-800 px-5 py-3 text-white flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-sm font-black">Smart Import: Hand Over Yet Confirm</h3>
          ...
          <div className="p-4 flex-1 flex flex-col gap-3">
            <p className="text-xs text-slate-600">
              Columns format expected: <strong>No | Code of handover minutes | Type of handover | Handover unit | Unit confirm handover | Handover date | Status</strong>
            </p>
            <textarea
              value={pasteData}
          ...
          <div className="bg-gradient-to-r from-rose-700 to-red-800 px-5 py-3 text-white flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-rose-200">Items pending confirmation longer than threshold</p>
              </div>
            </div>
                  ...
                  <div key={item.id} className="py-2.5 flex justify-between items-center text-xs hover:bg-rose-50/40 px-2 rounded">
                    <div>
                      ...
                      <div className="text-[11px] text-slate-600 flex gap-3">
                        <span>Handover: <strong>{item.handoverUnit}</strong></span>
                        <span>Confirm: <strong>{item.unitConfirm}</strong></span>
  ...
  // Floating Alarm Button (bottom-20 right-6 to prevent pagination overlap)
          ...
          <div className="flex justify-between items-center gap-2 flex-wrap">
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-sm font-black tracking-tight text-white flex items-center gap-1">
                  <span>⚠️</span> 03 HAND OVER_YET CONFIRM
                </h1>
            ...
            <div className="flex gap-1.5 items-center">
              <span className="text-slate-300 text-[10px] hidden lg:inline mr-2">
                <strong>METFONE NET:</strong> តាមដាន Hand Over មិនទាន់ Confirm
              </span>
          ...
          <div className="flex flex-wrap gap-2 justify-between items-center">
            ...
            <div className="flex gap-2 items-center flex-wrap">
              <div className="flex items-center gap-1 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded shadow-xs text-[10px]">
                ...
                <input type="number" value={alarmThreshold} onChange={(e) => setAlarmThreshold(parseInt(e.target.value, 10) || 3)} className="w-10 px-1 py-0 text-[10px] font-bold border border-amber-300 rounded text-center bg-white" min="1"/>
                <span className="font-bold text-amber-900">days</span>
              </div>
              <div className="relative">
                <input type="text" placeholder="Search code, unit, status..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-44 sm:w-56 px-2 py-0.5 text-[11px] font-medium border border-slate-300 rounded bg-white focus:ring-1 focus:ring-indigo-500 focus:border-transparent transition-all shadow-xs" />
              </div>
            </div>
          </div>
        </div>
              ...
              {paginatedData.map((item) => {
                ...
                return (
                  <tr key={item.id} className={`transition-colors ${isAlarm ? 'bg-rose-50/90 font-semibold' : selectedRows.has(item.id) ? 'bg-indigo-50/90' : 'even:bg-slate-50/70 odd:bg-white hover:bg-amber-50/80'}`}>
                    ...
                    <td className="border border-slate-300 px-1.5 py-0.25 font-mono font-bold text-slate-900 whitespace-nowrap min-w-[200px]">
                      {editingCell?.id === item.id && editingCell?.field === 'code' ? (
                        ...
                        <div onClick={() => startEdit(item.id, 'code', item.code)} className="cursor-pointer hover:bg-slate-200/60 px-1 py-0 rounded font-mono whitespace-nowrap">{item.code || '-'}</div>
                      )}
                    </td>
                    <td className="border border-slate-300 px-1.5 py-0.25 text-slate-800 whitespace-nowrap min-w-[170px]">
                      {editingCell?.id === item.id && editingCell?.field === 'type' ? (
                        <input type="text" defaultValue={item.type} autoFocus onBlur={(e) => saveEdit(item.id, 'type', e.target.value)} onKeyDown={(e) => handleKeyPress(e, item.id, 'type')} className="w-full px-1 py-0 border border-blue-500 rounded bg-white text-xs" />
                      ) : (
                        <div onClick={() => startEdit(item.id, 'type', item.type)} className="cursor-pointer hover:bg-slate-200/60 px-1 py-0 rounded whitespace-nowrap">{item.type || '-'}</div>
                      )}
                    </td>
                    <td className="border border-slate-300 px-1.5 py-0.25 text-slate-800 font-medium whitespace-nowrap min-w-[150px]">
                      {editingCell?.id === item.id && editingCell?.field === 'handoverUnit' ? (
                        <input type="text" defaultValue={item.handoverUnit} autoFocus onBlur={(e) => saveEdit(item.id, 'handoverUnit', e.target.value)} onKeyDown={(e) => handleKeyPress(e, item.id, 'handoverUnit')} className="w-full px-1 py-0 border border-blue-500 rounded bg-white text-xs" />
                      ) : (
                        <div onClick={() => startEdit(item.id, 'handoverUnit', item.handoverUnit)} className="cursor-pointer hover:bg-slate-200/60 px-1 py-0 rounded whitespace-nowrap">{item.handoverUnit || '-'}</div>
                      )}
                    </td>
                    <td className="border border-slate-300 px-1.5 py-0.25 text-slate-800 font-bold whitespace-nowrap min-w-[160px]">
                      {editingCell?.id === item.id && editingCell?.field === 'unitConfirm' ? (
                        <input type="text" defaultValue={item.unitConfirm} autoFocus onBlur={(e) => saveEdit(item.id, 'unitConfirm', e.target.value)} onKeyDown={(e) => handleKeyPress(e, item.id, 'unitConfirm')} className="w-full px-1 py-0 border border-blue-500 rounded bg-white text-xs" />
                      ) : (
                        <div onClick={() => startEdit(item.id, 'unitConfirm', item.unitConfirm)} className="cursor-pointer hover:bg-slate-200/60 px-1 py-0 rounded whitespace-nowrap">{getUnitConfirmBadge(item.unitConfirm)}</div>
                      )}
                    </td>
                    ...
                    <td className="border border-slate-300 px-1.5 py-0.25 text-center whitespace-nowrap">
                      <span className="inline-flex px-1.5 py-0 rounded font-mono font-bold text-purple-700 bg-purple-50 border border-purple-200">
                        {item.team || '-'}
                      </span>
                    </td>
...
export default HandOverYetConfirmMetfone;